...
class HybridSegmenter:
    ...
    def detect_segments_from_video(
        ...
        # Scale motion threshold based on resolution
        # Default threshold of 500000 is calibrated for 640x480 = 307200 pixels
        # Scale proportionally for different resolutions
        base_pixels = 640 * 480
        ...
        if verbose:
            ...
            print(f"Video resolution: {width}x{height} ({actual_pixels} pixels)")
            print(f"Motion threshold: {self.motion_threshold} (base) -> {effective_threshold:.0f} (scaled)")

        # First pass: collect all motion scores to determine adaptive threshold
        motion_scores = []
        prev_gray_pass1 = None
        ...
        while True:
            ...
            if not ret:
                break
            _, score, prev_gray_pass1 = self.detect_motion_in_frame(frame, prev_gray_pass1)
            motion_scores.append(score)

        # Calculate adaptive threshold based on motion score distribution
        # Also detect "valleys" (local minima) which indicate rest periods between signs
        valleys = []
        if motion_scores:
            sorted_scores = np.sort(motion_scores)
            min_score = sorted_scores[0]
            max_score = sorted_scores[-1]
            median_score = np.median(sorted_scores)

            # Use percentile-based threshold
            p10 = np.percentile(sorted_scores, 10)
            p25 = np.percentile(sorted_scores, 25)
            p75 = np.percentile(sorted_scores, 75)

            # Adaptive threshold based on the lower percentiles
            adaptive_threshold = p25

            if verbose:
                print(f"Motion score range: {min_score:.0f} - {max_score:.0f} (median: {median_score:.0f})")
                print(f"Percentiles: p10={p10:.0f}, p25={p25:.0f}, p75={p75:.0f}")
                print(f"Using adaptive threshold: {adaptive_threshold:.0f}")

            effective_threshold = adaptive_threshold

            # Detect valleys (local minima) - these are likely rest periods between signs
            # Smooth the scores first with a rolling average
            window_size = int(fps * 1.0)  # 1 second window (longer for stability)
            if window_size < 5:
                window_size = 5
            smoothed = np.convolve(motion_scores, np.ones(window_size)/window_size, mode='same')

            # Find local minima that are significantly lower than surrounding frames
            min_gap_between_valleys = int(fps * 1.0)  # At least 1 second between valleys
            last_valley = -min_gap_between_valleys

            for i in range(window_size, len(smoothed) - window_size):
                # Skip if too close to last valley
                if i - last_valley < min_gap_between_valleys:
                    continue

                local_region = smoothed[i-window_size:i+window_size+1]
                if smoothed[i] == np.min(local_region):
                    # Check if this minimum is significantly lower than the local max
                    local_max = np.max(local_region)
                    # Valley must be <20% of local peak (stricter threshold)
                    if smoothed[i] < local_max * 0.2:
                        valleys.append(i)
                        last_valley = i

            if valleys and verbose:
                print(f"Detected {len(valleys)} potential rest valleys at frames: {valleys[:10]}...")

        # Reset video for second pass
        cap.set(cv2.CAP_PROP_POS_FRAMES, 0)

        # Convert valleys list to a set for O(1) lookup
        valley_set = set(valleys) if valleys else set()
        ...
        while True:
            ...
            # Detect motion (returns raw motion score, not thresholded)
            _, motion_score, prev_gray = self.detect_motion_in_frame(
                frame, prev_gray
            )
            ...
            # Also check if we're at a valley (local minimum) - force rest detection
            ...
            if is_active:
                ...
                if in_sign:
                    ...
                    # Use shorter cooldown if we hit a valley (clear rest period)
                    required_cooldown = self.cooldown_frames // 3 if is_valley else self.cooldown_frames

                    if frames_since_motion >= required_cooldown or is_valley:
                        ...
                        if sign_duration >= self.min_sign_frames:
                            ...
                            if verbose:
                                valley_note = " (at valley)" if is_valley else ""
                                print(f"  Frame {sign_end}: Sign complete ({sign_duration} frames) -> segment {len(segments)}{valley_note}")
                        else:
                            if verbose:
            ...
            # Handle very long signs by splitting at valleys if available
            if in_sign and (frame_idx - sign_start) >= self.max_sign_frames:
                # Check if there's an upcoming valley we should split at instead
                upcoming_valleys = [v for v in valleys if sign_start < v <= frame_idx + 30]
                if upcoming_valleys:
                    split_frame = upcoming_valleys[0]
                else:
                    split_frame = frame_idx

                # Force end the current sign
                start_padded = max(0, sign_start - self.padding_before)
                end_padded = min(total_frames - 1, split_frame + self.padding_after)
                segments.append((start_padded, end_padded))

                if verbose:
                    print(f"  Frame {split_frame}: Sign too long, splitting -> segment {len(segments)}")
        ...
        if in_sign:
            ...
            # Check if this final segment is likely just end-of-video noise
            # If it starts shortly after the last valley and is short, skip it
            is_end_noise = False
            if valleys and sign_duration < self.min_sign_frames * 2:
                last_valley = valleys[-1]
                # If sign started within 30 frames after last valley, likely noise
                if sign_start > last_valley and sign_start - last_valley < 30:
                    is_end_noise = True
                    if verbose:
                        print(f"  End of video: Skipping short segment ({sign_duration} frames) - likely end-of-video noise")

            if not is_end_noise and sign_duration >= self.min_sign_frames:
                start_padded = max(0, sign_start - self.padding_before)
