...
# Add parent directory (project-utilities) to path for imports
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

# Try to import BLEU calculator
try:
...
try:
    ...
    print("SUCCESS: Quality scoring (GPT-2) available")
except ImportError:
    QUALITY_SCORING_AVAILABLE = False
    print("WARNING: Quality scoring not available (install with: pip install transformers torch)")
...
class SyntheticDatasetEvaluator:
    ...
    def __init__(
        ...
        # Initialize quality scoring models (lazy loading)
    ...
    def _calculate_gloss_accuracy(self, predicted_glosses, original_glosses):
        """
        Calculate gloss prediction accuracy by comparing predicted glosses to original input glosses

        Args:
            predicted_glosses: List of top-1 predicted glosses (strings)
            original_glosses: List of original input glosses (strings)

        Returns:
            dict with 'accuracy', 'correct', 'total', 'mismatches'
        """
    ...
    def _init_quality_scorer(self):
        """Initialize GPT-2 model for quality scoring (fluency/grammaticality)"""
        import os
        try:
            # Try to load from cache first (faster, no download)
            print("Loading GPT-2 model for quality scoring...")
            try:
                self.quality_tokenizer = GPT2Tokenizer.from_pretrained(
                    'gpt2',
                    local_files_only=True
                )
                self.quality_model = GPT2LMHeadModel.from_pretrained(
                    'gpt2',
                    local_files_only=True
                )
                print("SUCCESS: Loaded GPT-2 from cache")
            except Exception as cache_error:
                # Not in cache or cache is corrupted, need to download
                print("Cache not found or corrupted, downloading GPT-2 model (first time only, ~548MB)...")
                print("This will be cached for future runs. Please wait...")
                try:
                    # Force re-download and overwrite corrupted cache
                    self.quality_tokenizer = GPT2Tokenizer.from_pretrained(
                        'gpt2',
                    ...
                    print(f"ERROR: Download failed: {download_error}")
                    raise

            self.quality_model.eval()  # Set to evaluation mode

            # Set pad token (GPT-2 doesn't have one by default)
            self.quality_tokenizer.pad_token = self.quality_tokenizer.eos_token

            print("SUCCESS: Quality scorer is ready for evaluation")
        except Exception as e:
            print(f"ERROR: Failed to initialize quality scorer: {e}")
            print("Quality scores will not be available for this evaluation run.")
            print("\nTIP: If you see cache-related errors, try deleting the cache directory:")
            print("     C:\\Users\\<username>\\.cache\\huggingface\\hub\\models--gpt2")
            traceback.print_exc()
            self.quality_model = None
            self.quality_tokenizer = None

    def _calculate_quality_score(self, sentence):
        """
        ...
        """
        if not QUALITY_SCORING_AVAILABLE or self.quality_model is None:
            return None
        ...
        try:
            ...
            with torch.no_grad():
                outputs = self.quality_model(**inputs, labels=inputs['input_ids'])
                loss = outputs.loss
                perplexity = torch.exp(loss).item()

            # Debug: print perplexity
            ...
            # Observed range: 50-5000+
            # Using log scale to handle wide range:
    ...
    def _calculate_gloss_coverage(self, reference_sentence, output_sentence):
        """
        Calculate semantic coverage metrics (comparative measure)
        ...
        Args:
            reference_sentence: Reference sentence (e.g., "The dog will walk now.")
            output_sentence: Generated sentence (e.g., "The dog is walking now.")

        Returns: dict with 'recall', 'precision', 'f1', 'missing_words', 'hallucinated_words'
        """
        try:
            ...
            for ref_word in reference_words:
                ...
                for out_word in output_words:
                    ...
                    if ref_word in out_word or out_word in ref_word or out_word.startswith(ref_word[:4]) and len(ref_word) >= 4:
                        found = True
                        break
    ...
    def _calculate_baseline_metrics(self, predicted_top1_glosses, original_glosses, reference_sentence):
        ...
        # Calculate BLEU directly using sacrebleu (no dataset lookup needed)
        if BLEU_AVAILABLE:
            try:
                # Normalize both sentences (case-insensitive, punctuation-insensitive)
                translator = str.maketrans('', '', string.punctuation)

                baseline_normalized = baseline_sentence.translate(translator).strip().lower()
                reference_normalized = reference_sentence.translate(translator).strip().lower()

                # Calculate BLEU score directly
                ...
                if not self.quiet:
                    print(f"  ERROR: Baseline BLEU calculation failed: {e}")

        # Calculate BERTScore
        if BERTSCORE_AVAILABLE:
            try:
                P, R, F1 = bert_score([baseline_sentence], [reference_sentence], lang="en", verbose=False)
                bertscore_f1 = F1.item() * 100  # Convert to 0-100 scale like BLEU
                if not self.quiet:
                    print(f"  BASELINE BERTScore: {bertscore_f1:.2f}")

            except Exception as e:
                if not self.quiet:
                    print(f"  ERROR: Baseline BERTScore calculation failed: {e}")

        # Calculate Quality Score (reference-free)
        quality_score = self._calculate_quality_score(baseline_sentence)
        if quality_score is not None and not self.quiet:
            print(f"  BASELINE Quality: {quality_score:.2f}")

        # Calculate Coverage (baseline sentence vs reference sentence)
        coverage = self._calculate_gloss_coverage(reference_sentence, baseline_sentence)
    ...
    def _calculate_model_metrics(self, predicted_glosses, predicted_sentence, reference_sentence):
        ...
        if BLEU_AVAILABLE:
            try:
                ...
                predicted_normalized = predicted_sentence.translate(translator).strip().lower()
                reference_normalized = reference_sentence.translate(translator).strip().lower()
                ...
                if not self.quiet:
                    print(f"  ERROR: Model BLEU calculation failed: {e}")
                    traceback.print_exc()

        # Calculate BERTScore
        if BERTSCORE_AVAILABLE:
            try:
                P, R, F1 = bert_score([predicted_sentence], [reference_sentence], lang="en", verbose=False)
                bertscore_f1 = F1.item() * 100  # Convert to 0-100 scale like BLEU
                if not self.quiet:
                ...
                if not self.quiet:
                    print(f"  ERROR: Model BERTScore calculation failed: {e}")

        # Calculate Quality Score (reference-free)
        quality_score = self._calculate_quality_score(predicted_sentence)
        if quality_score is not None and not self.quiet:
            print(f"  MODEL Quality: {quality_score:.2f}")

        return bleu_score, bertscore_f1, quality_score

    def _calculate_composite_score(self, bleu, bertscore, quality, coverage_f1):
        """
        Calculate composite score using weighted average
        All scores should be on 0-100 scale
        Returns: composite_score (0-100) or None if insufficient data
        """
        scores = {}
        weights_sum = 0
        ...
        composite = sum(scores[key] * METRIC_WEIGHTS[key] / weights_sum
                       for key in scores)
    ...
    def evaluate_entry(self, entry, entry_id):
        ...
        try:
            ...
            gloss_accuracy_result = self._calculate_gloss_accuracy(top1_glosses, glosses)
            ...
            result['baseline_composite'] = self._calculate_composite_score(
                baseline_bleu, baseline_bertscore, baseline_quality, baseline_coverage['f1']
            )
            ...
            model_coverage = self._calculate_gloss_coverage(reference_sentence, predicted_sentence)
            ...
            result['model_composite'] = self._calculate_composite_score(
                model_bleu, model_bertscore, model_quality, model_coverage['f1']
            )
    ...
    def evaluate_dataset(self, limit=None):
        ...
        print(f"  Quality Score (GPT-2): {'Enabled' if (QUALITY_SCORING_AVAILABLE and self.quality_model is not None) else 'DISABLED'}")
        print(f"  Composite Score: {'Enabled' if (BLEU_AVAILABLE or BERTSCORE_AVAILABLE or (QUALITY_SCORING_AVAILABLE and self.quality_model is not None)) else 'DISABLED'}")
        print("="*80)
